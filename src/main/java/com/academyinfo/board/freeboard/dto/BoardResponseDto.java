...
public class BoardResponseDto {
	...
	private String mid;
	private List<FreeboardImageResponseDto> iindex;
	private List<CommentResponseDto> cmtindex;
	
	
	@Builder
	public BoardResponseDto(int fbindex, String title, String contents, int count, int report, LocalDateTime createdDate, LocalDateTime modifiedDate, MemberEntity mindex, List<FreeboardImageEntity> iindex, List<CommentEntity> cmtindex) {
		...
		this.iindex = iindex.stream().map(FreeboardImageResponseDto::new).collect(Collectors.toList());
	...
	public BoardResponseDto(BoardEntity boardEntity) {
		...
		// List 타입을 DTO 클래스로 하여 엔티티간 무한참조 방지
		this.iindex = boardEntity.getIindex().stream().map(FreeboardImageResponseDto::new).collect(Collectors.toList());
